            ...
            while not coord.should_stop():
                ...
                while not d:
                    ...
                    if random.random() > epsilon:
                        ...
                        #a_dist = sess.run([self.local_AC.out], feed_dict={self.local_AC.inputs:k_frames[None,:]})
                    ...
                    if d != True and len(buffer_a) == 10:
                        ...
                        l, g_n, v_n, v1 = self.train(batch_memory, sess, gamma)
                        #l, g_n, v_n, v1 = self.train(rollout, sess, gamma)
                        """
                        bs, ba, br = np.vstack(buffer_s), np.vstack(buffer_a), np.array(buffer_r)[:, np.newaxis]
