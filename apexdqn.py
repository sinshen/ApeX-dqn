...
class AC_Network():
    def __init__(self,sess, s_size,a_size,scope,trainer):
        with tf.variable_scope(scope):
            ...
            if scope == 'global':
                ...
                self.loss = tf.reduce_mean(self.ISWeights * tf.square(y - readout_action))
    ...
    def train(self,sess,gamma):
        while not coord.should_stop():
            ...
            observations = batch_memory[0]
            actions = batch_memory[1]
            rewards = batch_memory[2]
            v1 = batch_memory[3]
        ...
        with sess.as_default(), sess.graph.as_default():
            while not coord.should_stop():
                ...
                while not d:
                    ...
                    if d != True and len(episode_buffer) == 10:
                        sess.run(self.update_local_ops)
                    if d == True:
                        break
