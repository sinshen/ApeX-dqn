...
class Worker():
    ...
    def train(self,rollout,sess,gamma):
        ...
        v1 = sess.run(self.local_AC.out, feed_dict={self.local_AC.inputs:np.vstack(s1)})#[0]
        print 'ppp', v1
        v1 = np.max(v1,axis=1)
        print 'jjj', v1
        feed_dict = {self.local_AC.target_v:v1,
            ...
            while not coord.should_stop():
                ...
                while not d:
                    ...
                    if d != True and len(episode_buffer) == 10 and replay_buffer.can_sample(batch_size):
                        ...
                        rollout = [obs_t_batch, act_batch, rew_batch, obs_tp1_batch, done_mask]
